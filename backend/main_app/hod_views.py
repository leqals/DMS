...
def admin_home(request):
    ...
    department_name = Department.objects.all()[0].name if len(Department.objects.all()) else 'No Department Assigned'
    ...
    for course in courses:
        ...
        course_list.append(course.name[:7])
...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            title = student_form.cleaned_data.get('title')
            dep = student_form.cleaned_data.get('dep')
            date_of_birth = student_form.cleaned_data.get('date_of_birth')
            maiden_name = student_form.cleaned_data.get('maiden_name')
            matric_number = student_form.cleaned_data.get('matric_number')
            religion = student_form.cleaned_data.get('religion')
            marital_status = student_form.cleaned_data.get('marital_status')
            health_status = student_form.cleaned_data.get('health_status')
            ...
            try:
                ...
                user.student.dep = dep
                user.student.date_of_birth = date_of_birth
                user.student.maiden_name = maiden_name
                user.student.matric_number = matric_number
                user.student.religion = religion
                user.student.marital_status = marital_status
                user.student.health_status = health_status
                user.save()
                messages.success(request, "Successfully Added")
...
def add_department(request):
    ...
    if len(dep) >= 1: return redirect(reverse('manage_dep'))
    else:
        context = {
            'form': form,
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                messages.error(request, "Could Not Add")
    return render(request, 'hod_template/add_dep_template.html', context)
...
def add_course(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            course_units = form.cleaned_data.get('course_units')
            try:
                course = Course()
                course.name = name
                course.staff = staff
...
def manage_dep(request):
    ...
    return render(request, "hod_template/manage_dep.html", context)
...
def edit_dep(request, dep_id):
    instance = get_object_or_404(Department, id=dep_id)
    ...
    return render(request, 'hod_template/edit_dep_template.html', context)


def edit_subject(request, subject_id):
    instance = get_object_or_404(Course, id=subject_id)
    ...
    if request.method == 'POST':
        if form.is_valid():
            name = form.cleaned_data.get('name')
            course = form.cleaned_data.get('dep')
            staff = form.cleaned_data.get('staff')
            try:
...
def delete_course(request, dep_id):
    ...
    return redirect(reverse('manage_dep'))
