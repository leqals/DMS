...
def student_home(request):
    ...
    total_subject = Course.objects.filter(dep=student.dep).count()
    ...
    data_present = []
    data_absent = []
    subjects = Course.objects.filter(dep=student.dep)
...
def student_assignments(request):
    ...
    for assignment in assignments:
        try:
            ...
            submission = [i for i in submission]
        finally: pass
        
        if submission:
            if not type(submission) == 'array': done_assignments.append(submission)
            done_assignments = done_assignments + submission
        else:
            undone_assignments.append(assignment)
    
    context = {
        'page_title': page_title,
        'done_assignments': done_assignments,
        'undone_assignments': undone_assignments,
    }
    
    return render(request, 'student_template/assignments.html', context)

def view_submission(request, pk):
    submission = get_object_or_404(Submission, id=pk)
    page_title = f'Check out {submission}'
    ...
    return render(request, 'student_template/view_submission.html', context)
...
def submit_assignment(request):
    ...
    if request.method == 'POST':
        ...
        form = SubmissionForm()
    return render(request, 'student_template/submit_assignment.html', {'form': form})
...
def student_view_attendance(request):
    student = get_object_or_404(Student, admin=request.user)
    if request.method != 'POST':
        dep = get_object_or_404(Department, id=student.dep.id)
        context = {
